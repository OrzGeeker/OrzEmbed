...
为避免**手工逐脚短接**验收,仓库内置自动自检固件 `project/hwtest` 与脚本 `scripts/esp32c6-hwtest.sh`,
**一条命令跑完所有外设并输出 PASS/FAIL**:
...
覆盖项与结果(2026-09-22 实测):

| 项 | 结果 | 说明 |
|----|------|------|
| chip | PASS | ESP32-C6,rev v0.1 |
| flash | PASS | 4MB,JEDEC `0x204016` |
| nvs | PASS | 写入 + commit + 回读一致 |
| **gpio_bridge** | PASS | 排针间 **0 处锡桥/短路** |
| gpio_pull | PASS | 仅 GPIO22(LCD 背光 10K 下拉)、SPI/SD、BOOT 等**已知**外部上下拉 |
| lcd | PASS | ST7789 172×320 初始化 + 画面(肉眼) |
| sd | PASS | `SA08G`,1MB 写入 + 回读校验一致 |
| rgb | PASS | WS2812 G/R/B(肉眼) |
| wifi | WARN | 仅收到 `MyAP` @ **-96 dBm**(弱,见第 4 节) |
...
|--------|------------------|----------|------|
| 接收同一 AP `MyAP`(ch6) | **-35 dBm** | **-91 ~ -97 dBm** | **约 55~60 dB** |
| 附近 AP 数量 | 10 | 0~1 | — |
...
- 重新焊接天线 `C3` 后复测:ESP32 收 `MyAP`(ch6)**-94 ~ -100 dBm**,Mac 同期 **-38 ~ -41 dBm**,仍差 **~55~60 dB**。
- 万用表测 `C3` 两端电阻很小 → **属正常**(多数 2.4G 陶瓷天线馈点↔地内部即为直流短路),不能据此判定天线损坏。
- 结论:`C3` 不是(或不止是)病根;全频段 60dB 衰减更像 RF 通路**在别处失谐/断裂**,或陶瓷天线内部开裂。下一步按第 5 节做“搭线法”或换天线验证。
- 另:焊接排针后曾出现**启动异常(无输出/反复进下载模式)**,排查为排针处焊接问题,修好后恢复正常;`gpio_bridge` 自检确认排针间无短路。
